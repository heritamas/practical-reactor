...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void plan_b() {
        //todo: feel free to change code as you need
        Flux<String> stonks = Flux.concat(getStocksLocalCache(), getStocksRest());
    ...
    public void mail_box_switcher() {
        ...
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst(
                (signal, flux) -> signal.get().metaData.equals("spam") ? mailBoxSecondary() : flux
        );
    ...
    public void cleanup() {
        ...
        Flux<String> stream = Flux.usingWhen(
                ...
                Function.identity(),
                d -> StreamingConnection.closeConnection()
        );
